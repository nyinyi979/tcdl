...
export default function Navbar(props:{currentLink:string}){
    ...
    return(
        <>
        <div id='navbar' className="lg:relative lg:h-fit lg:py-12 py-4 lg:bg-transparent sticky z-10 top-0 right-0 h-16 px-2 w-full float_clear bg-black/80 opacity-0">
            ...
            <div className='float-right xll:pr-8 xl:pr-5 lg:pr-2 pr-0'>
                ...
                <div className='float-right xll:pt-12 lg:pt-2 lg:mt-2'>
                    ...
                    <div className="lg:hidden block float-left ml-3 w-fit h-[2rem] text-white cursor-pointer hover:text-primary" onClick={()=>{Drawer.restart()}}><IoMdMenu className="text-3xl"/></div>
                </div>
            </div>

        </div>
        
            <div id='drawers' className='lg:hidden fixed right-[-400px] hidden top-0 bg-violet w-64 h-full z-10'>
                <div className='inline-block w-fit h-fit text-white text-xl p-3 hover:text-primary cursor-pointer' onClick={()=>{DrawerOutro.restart()}}><ImCross /></div>
